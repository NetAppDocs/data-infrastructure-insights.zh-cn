...
此计算机不应运行任何其他应用程序级别的软件。建议使用专用服务器。

如果您正在使用 SELinux 运行，建议在采集单元系统上执行以下命令：
...
对于 Linux，磁盘空间应按以下方式分配：

* /opt/netapp 10 GB（大型环境为 20 GB）
* /var/log/netapp 40 GB（大型环境为 80 GB）
* /tmp 在安装过程中至少有 1 GB 可用


您的文件系统可能会使用一些空间作为其开销。这些是在安装之前应该 _可用_ 的最小空间。
| 50 GB 
