...
summary: 部署工作负载安全代理来监控用户活动、检测安全威胁并保护您的存储基础架构。 
---
= 部署工作负载安全代理
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
工作负载安全代理对于监控用户活动和检测存储基础架构中潜在的安全威胁至关重要。本指南提供分步安装说明、代理管理最佳实践（包括暂停/恢复和固定/取消固定功能）以及部署后配置要求。开始之前，请确保您的代理服务器满足以下条件： link:concept_cs_agent_requirements.html["系统要求"]。
...
| 别针和拔针 | Unpin 会立即获取最新版本（如果可用），并升级代理和收集器。在此升级过程中，fpolicies 将断开连接并重新连接。此功能专为希望控制自动升级时间的客户而设计。请见下文<<pinning-an-agent-at-the-current-version,插针/拔针说明>>。 
