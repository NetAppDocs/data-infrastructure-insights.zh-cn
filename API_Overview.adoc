...
* ASSETS 类型包含资产、查询和搜索 API。DII 资产属性和指标有时被称为"odata"。这些 API 通常通过 /rest/v1/asset 或 /rest/v1/query 访问。有关详细信息，请参见 <<api-documentation-swagger,API Swagger 文档>>。
+
...
* 数据收集类型用于检索和管理数据收集器。
* DATA INGESTION 类型用于检索和管理摄取数据和自定义指标，例如来自 Telegraf 代理的数据和自定义指标。这些指标有时被称为"数据湖"或"湖泊"指标。这些 API 通常通过 /rest/v1/lake 访问。有关详细信息，请参见 <<api-documentation-swagger,API Swagger 文档>>。
* LOG INGESTION 用于检索和管理日志数据


随着时间的推移，可能会有更多类型和/或 API 可用。您可以在link:#api-documentation-swagger["API Swagger 文档"]。

请注意，用户有权访问的 API 类型也取决于他们在每个 Data Infrastructure Insights 功能集（可观察性、工作负载安全性、报告）中拥有的link:concept_user_roles.html["用户角色"]。
