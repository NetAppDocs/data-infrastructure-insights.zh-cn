...
summary: 通过将性能、容量、配置和组件警报整合到可操作的运行状况评分中的全面监控，获得存储环境运行状况的统一视图。 
---
= 监控基础设施健康状况
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
Data Infrastructure Insights 通过跟踪性能、容量、配置和组件状态来监控存储环境的运行状况。系统根据这些类别的监控警报计算统一的运行状况分数，从而清晰呈现系统运行状况，并支持主动解决问题。
