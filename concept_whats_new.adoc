...
除了以下操作系统之外、Data Infrastructure Insight采集单元现在还支持这些操作系统link:concept_acquisition_unit_requirements.html["已支持"]：
...
DII 的 *VM Analyzer* 功能提供全面的端到端虚拟化环境视图。VMAnalyzer 通过映射从虚拟机到存储的整个路径来简化故障排除，使用户能够快速查明虚拟磁盘、虚拟机、主机、数据存储区、卷和存储中的性能问题。借助直观的拓扑视图、筛选功能和详细的性能指标，管理员可以轻松识别瓶颈、分析流量不平衡并优化整体存储性能。

image:vm_analyzer_example_with_panel.png["存储系统的VM分析器拓扑图"]
...
重新设计的登录页面可在一个视图中显示您所需的一切—摘要数据、性能图表、相关性、警报和更改以及关键资源。

现在、您可以更快地排除故障并保持流畅状态、而不会在选项卡之间跳转或丢失上下文。

image:lp_new_design.png["新的登录页面设计、显示摘要、性能和其他资源部分的位置"]



=== 工作负载安全性改进

*Webhook现在可用于工作负载安全警报*

工作负载安全性现在支持向Slack, PagerDty和团队等工具发送即装即用的webhook通知。此外、我们还提供可自定义的模板、使您可以定制消息或与任何其他暹粒或第三方应用程序集成。将关键警报直接路由到现有安全工作流、以便您的团队可以更快地进行调查和响应。

image:ws_webhook_slack_example.png["工作负载安全性的webhook可宽延时间示例、宽度=400"]

*跨代理迁移工作负载安全性收集器*

您可以轻松地将工作负载安全收集器从一个代理迁移到另一个代理、从而在代理之间实现高效的收集器负载平衡。迁移与编辑收集器和从列表中选择目标代理一样简单。
...
将数据导出到.CSV可能需要几秒钟到几小时、具体取决于要导出的数据量。现在、Data Infrastructure Insight可以异步导出这些数据、因此您可以在编译.CSV时继续工作。

选择右上角工具栏中的"铃"图标、查看.CSV导出。

image:csv_export_async.png["钟形图标以及.csv导出列表、可供下载、宽度为400"]
