...
将复杂数据即时转换为图形洞察。请 AI Assistant 创建仪表板，它将通过交互式图形和表格可视化答案，帮助您识别趋势并更快地解决问题，而无需手动创建图表。

image:ai_assistant-column_graph.png["AI Assistant 可视化"]
...
VM Analyzer 视图中的新开关允许您隐藏没有数据存储的已挂载卷，从而提供更整洁的视图以简化 VM 故障排除。

image:vm_analyzer_datastore_only_view.png["仅数据存储视图滑块"]
...
保持资产信息准确，同时降低意外更改的风险。注释和属性（即"用户数据"）现在默认以只读模式显示，因此您可以先查看详细信息，然后在准备好后选择编辑。

仅在需要时进行编辑。从滑出面板中，展开资源详细信息并选择铅笔图标。资源登录页面将在"编辑"模式下打开，您可以在其中更新信息，完成后选择保存。

您还可以直接打开资产登录页面，然后选择右上角的"编辑"。

image:UserDataEdit-Slideout.png["在侧拉面板中编辑用户数据-铅笔图标，宽度=400"]

image:UserDataEdit-LP.png["登录页编辑模式，width=200"]
...
DII AI Assistant 现在具有专用的起始页面，将智能基础设施分析置于核心位置。即时开始新对话，一键恢复上一个会话，或使用建议的提示快速开始。

image:AI_dashboard_page_default.png["AI 仪表板默认，width=800"]

从导航菜单中的 *AI Assistant* 访问起始页面，或使用左上角的"星星"图标从任何位置打开 AI Assistant。

image:AI_dashboard_page_menu.png["AI 助手导航菜单，宽度=600"] image:AI_assistant_access_button.png["AI Assistant 可随时随地访问，width=300"]
...
通过可自定义的登录消息，让您的团队了解最新情况并保持一致。在用户登录时，直接向其传达重要公告、维护计划或策略更新。借助 Markdown 支持，您可以设置消息格式以确保清晰易读，并确保关键信息传达给每位 DII 用户。具有管理员权限的帐户可以在"通知"下的管理菜单中找到相关配置。
...
DII 导航菜单具有全新的外观，现在与 Console 的颜色和行为相匹配，以获得更统一的体验。先前静态的主菜单默认情况下将折叠，以获得更集中的工作空间和有效使用屏幕空间。只需点击左上角的汉堡图标（☰）即可访问。
...
通过人工智能驱动的设备识别加速新设备的引入。Data Infrastructure Insights 分析您的环境并提出自动设备解析规则，帮助您快速识别和管理更多设备。只需查看和批准 AI 生成的规则，DII 即可处理其余部分，从而提高整个基础设施的设备可见性和管理。
...
AI Assistant 现在支持十几种语言，使您的团队能够自然地使用他们的首选语言。查看完整列表link:https://platform.claude.com/docs/en/build-with-claude/multilingual-support["此处"^]。
...
属性监视器现在可用于在资产在数据中心之间移动、存储池未加密或发生任何属性修改时自动提醒您的团队。这有助于维护配置基线，并有助于确保在新部署或更改期间不会出现任何漏洞。

image:attribute_monitor_in_list.png["属性监视器已添加到监视器类型中"]



[[build-smarter-alerts-and-webhooks-with-dynamic-context-rich-messages]]
=== 使用动态、上下文丰富的消息构建更智能的警报和 webhook

创建监视器或 webhook 时，只需在警报描述或 webhook URL、标题或正文中键入"%"，即可从动态填充的可用对象属性和注释列表中进行选择。为您的警报提供正确的上下文，以提高清晰度、增强 webhook 集成并加快响应时间。

image:attribute_list_example.png["监视器描述中的属性列表，width=400"]
...
=== Data Infrastructure Insights 比以往任何时候都更了解您的环境

我们对 Data Infrastructure Insights 中的 AI 如何理解您的设置进行了增强。在响应之前，它会利用有关您当前服务使用情况、配置和性能模式的更丰富上下文。其结果是，初始指导反映了您的环境，而不是通用起点，需要更少的后续跟进即可获得有用的答案。

例如，导航到存储设备，AI 助手将为以下问题提供一键式答案：

* 此存储的容量利用率趋势如何，哪些 StoragePools 占用空间最多？
* 您可以告诉我过去 24 小时内此设备的容量利用率趋势吗？
* 哪些卷在此存储上占用的空间最多？
...
Data Infrastructure Insights AI Assistant 现在已正式向托管在 AWS 上的 DII 站点的所有客户提供。

请注意，Azure 网站上的 AI Assistant 仍处于预览状态。敬请关注此空间，了解正式发布的公告。



[[infrastructure-insights-features-removed]]
=== 已删除 Infrastructure Insights 功能

DII 的 "Infrastructure Insights" 功能 _Shared Resources Under Stress_、_Kubernetes Namespaces Running out of Space_ 和 _Reclaiming ONTAP Cold Storage_ 在 DII 中不再可用。
...
认识基础设施的新智能合作伙伴：link:ai_for_dii_overview.html["*Data Infrastructure Insights AI 助手*"] 转变您与数据交互的方式，将复杂的查询转化为简单的对话。即时发现隐藏的模式，加快故障排除，并自信地做出数据驱动的决策。获得以下问题的即时答案：
...
DII AI Assistant Preview 现在可供大多数客户使用，服务托管在 AWS 区域；Azure 区域即将推出。link:concept_preview_features.html["了解更多"] 关于 Data Infrastructure Insights 预览功能。

image:ai_assistant_icon.png["AI Assistant 图标"]
...
直接修改默认系统监视器以满足您的需求——在一个地方更改名称、过滤器和分组。无需为简单调整创建重复监视器。一个有用的横幅提醒您已定制了哪些监视器，从而保持对监控配置的完全可见性。



==== 通过批量操作简化监视器管理

通过批量操作一次管理多个监视器。快速调整多组监视器的运行状况监控设置，节省您的时间并降低复杂性。在几秒钟内而不是几分钟内全面更新您的监控策略。



==== 将您的基础架构健康评分重点放在最重要的事情上

从基础架构运行状况分数中排除特定对象，以保持准确性和相关性。查看基础架构运行状况仪表板时，立即删除要取消优先级的对象，而无需修改监视器配置。这会将警报集中在推动业务优先级的基础设施元素上。

image:health_monitoring_exclude_object.png["从运行状况监控中排除对象"]
...
Data Infrastructure Insights link:concept_acquisition_unit_requirements.html["采集单元"] 目前支持以下 Linux 操作系统：

* AlmaLinux 9.5 和 10
* Centos（64 位）：Stream 9
* Debian（64 位）：11 到 13
* OpenSUSE Leap 15.2 至 15.6、16
* Oracle Enterprise Linux (64 位)：8.10、9.2、9.4、9.6、10
* Red Hat (64 位)：8.10、9.2、9.4、9.6、10
* Rocky 9.2 到 9.6
...
使用细粒度的权限控制来共享您创建的查询。选择只读查看器模式以防止修改，或选择编辑模式以允许更改。私有查询可供创建者使用，而共享查询可在注释规则中使用，或复制用于特殊用例，让您可以完全控制查询在整个组织中的使用和修改方式。

image:query_sharing_dialog.png["设置查询权限，宽度=200"]

此外，具有_Guest_角色的用户现在可以创建和管理自己的私有查询。
...
对监控列表和组的筛选功能进行了改进，增加了通配符支持、多个筛选器、包含/排除功能等。

image:monitor_list_filters.png["新显示器筛选，宽度=500"]
...
改进后的仪表盘设计可帮助您查看最重要的信息。通过阴影边框分隔小部件，并移除分隔线，增强视觉清晰度，减少杂乱，让您可以专注于数据洞察。这些改进创造了更简洁、更直观的仪表盘体验，使用户能够更快、更容易地发现趋势和异常情况。
...
Reclaim ONTAP Cold Storage Insight 现在支持 FlexGroups，现在可供所有客户使用。
...
| Kubernetes 命名空间空间不足 | _Kubernetes Namespaces Running Out of Space_ Insight 为您提供 Kubernetes 命名空间上存在空间耗尽风险的工作负载视图，并估算每个空间占满之前的剩余天数。 


| 共享资源面临压力 | _Shared Resource Under Stress_ insight 使用 AI/ML 自动识别资源争用导致环境中性能下降的位置，突出显示受其影响的任何工作负载，并提供建议的补救措施，让您能够更快地解决性能问题。 
...
Cloud Insights 定期推出许多令人兴奋的新预览功能。如果您有兴趣预览其中一项或多项功能，请联系您的 NetApp 销售团队以获取更多信息。
...
| 共享资源面临压力 | 压力下的共享资源洞察使用 AI/ML 自动识别资源竞争导致环境中性能下降的位置，突出显示受其影响的任何工作负载，并提供建议的补救措施，让您能够更快地解决性能问题。 
|===
