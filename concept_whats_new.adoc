...
介绍我们旨在增强您的安全性和资源管理能力的最新功能！借助高级取证分组依据功能和分层多组依据支持、您现在可以轻松识别哪些用户访问了特定文件夹、确定最活跃的用户和共享、并通过跟踪活动客户端IP地址主动管理风险。通过确定访问量最多的文件和文件夹来优化存储和带宽使用、并通过识别实际用户来增强对系统访问的控制。

image:forensics-group-by-example-screen.png["显示分组依据的取证示例屏幕"]
